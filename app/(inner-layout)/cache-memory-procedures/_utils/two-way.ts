...
export const handleCalculateTwoWayAssociative = (
  addressBits: number,
  blocks: number,
  unusedBlocks: string[],
  offsetBits: number,
  ...
  const tagLabel = `tag ${addressBits - (indexBits - 1) - offsetBits}b`;
  const indexLabel = `set ${indexBits - 1}b`;
  const offsetLabel = `offset ${offsetBits}b`;
  for (const block of unusedBlocks) {
    try {
      const address = evaluate(block);
      const tag = Math.floor(address / Math.pow(2, offsetBits + indexBits - 1));
      const index = Math.floor(
        (address / Math.pow(2, offsetBits)) % Math.pow(2, indexBits - 1),
      );
